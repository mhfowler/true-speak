...
// this is meant to pass javascipt gmail api wrapper back to content script (from the page)
$(document).ready(function() {
    // TODO: only trigger when inbox page, or click to next page of inbox
    setTimeout(function() {
        sendMessageToDecryptPreviews();
    }, 0);
...
function sendMessageToDecryptPreviews() {
    var visible_inbox_data = getVisibleInboxData();
       window.postMessage({
                type: "PT_MESSAGE",
                text: "Hello from the page's javascript!",
                visible_inbox: visible_inbox_data
            },
            '*' /* targetOrigin: any */);
}


function getVisibleInboxData() {
    var gmail_api = Gmail();
    var to_return = {};
    try {
        var visible_emails = gmail_api.get.visible_emails();
    }
    catch (err) {
        return {};
    }
    $.each(visible_emails, function(i,e) {
        var e_id = e.id;
        var e_preview = e.excerpt;
        var FAKEBLOCK_OPEN_TAG = "Email encrypted by ParselTongue";
        var email_content = "";
        var is_match = (e_preview.substring(0, FAKEBLOCK_OPEN_TAG.length) == FAKEBLOCK_OPEN_TAG);
        try {
            if (is_match) {
                var full_email = gmail_api.get.email_data(e_id);
                email_content = full_email.threads[e_id].content_html;
            }
            to_return[i] =
            {"email_id":e_id,
            ...
            console.log("failed to get preview content?");
            to_return[i] =
            {"email_id":e_id,
                "email_content":""
            };
